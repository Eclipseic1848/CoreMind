...
TUI 忙碌期间，普通 Enter 输入不会被排队到下一轮；`/abort` 与 `/children` 仍可立即执行。
...
While the TUI is busy, ordinary Enter input is not queued into a later turn; `/abort` and `/children` remain available immediately.
