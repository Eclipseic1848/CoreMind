...
`);
  ...
  it("解析 JSON 配置（JSON 是 YAML 超集）", () => {
    const data = parseConfigText(
      JSON.stringify({ version: 1, name: "json-demo", agents: { main: {} } }),
    );
    expect(data).toMatchObject({ name: "json-demo" });
  });

  it("语法错误抛出 ConfigParseError", () => {
    expect(() => parseConfigText("version: [1, 2")).toThrow(ConfigParseError);
  });

  it("YAML 与 JSON 解析后走同一校验路径", () => {
    const yamlData = parseConfigText("version: 1\nname: demo\nagents:\n  main: {}\n");
    const jsonData = parseConfigText('{"version":1,"name":"demo","agents":{"main":{}}}');
