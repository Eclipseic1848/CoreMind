...
- Windows P20 首轮发现人工拒绝后模型会把拒绝当成普通工具错误并重复申请审批。Runtime 现在在第一次人工拒绝后立即终止当前工具批次并返回 `paused`；回归测试锁定只允许 1 次模型请求、1 次审批和零副作用。修复后全仓为 461 项通过、4 项按条件跳过。
- 修复后三轮稳定性均为 461 项通过、4 项条件跳过且零漂移；覆盖率 lines 75.78%、statements 73.91%、functions 83.15%、branches 66.87%，不下降门禁通过。总体 80% 与部分关键安全分支 90% 仍是长期目标。
