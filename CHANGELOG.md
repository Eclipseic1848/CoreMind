...
- Child Run 未显式配置单次输出上限时沿用所选模型的上限，并受子任务 token 预算约束，避免把累计预算误当作模型输出能力。
...
- 根版本、8 个 npm 包、内部精确依赖、Python SDK、bundled Worker、模块合同、文档和 Release Please 元数据统一为稳定版 `0.7.0`。源码开发与 CI 固定 npm `11.5.1`，避免已退役的旧审计接口误拒绝有效 workspace 树。`v0.7.0` Tag、[GitHub Release](https://github.com/Eclipseic1848/CoreMind/releases/tag/v0.7.0)、8 个 npm 包与 [PyPI](https://pypi.org/project/coremind-ai/0.7.0/) 已从同一版本同步发布。
