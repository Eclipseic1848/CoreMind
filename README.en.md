...
> Current development source (not yet published) adds reconstructible ReplayKit evidence and always-visible local Observability while keeping Telemetry egress explicit, independent, and disabled by default. It adds no OTel dependency and has not run a live OTLP endpoint, credentials, or Provider certification. Registry and Release artifacts remain authoritative for published `0.3.1` behavior.
