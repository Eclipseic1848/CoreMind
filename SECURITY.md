...
当前源码加固（尚未发布）：内置文件工具的审批、Checkpoint 与执行共享解析后的目标，并在执行前复核；这不等于 OS 沙箱，不能完全防止恶意宿主进程制造文件系统竞态。Git 工具要求 Git 2.36+，禁用 fsmonitor、外部 diff/textconv 等辅助执行，默认读取范围限制在当前工作目录；不把仓库配置视作可信代码。
