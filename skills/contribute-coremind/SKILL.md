...
8. Run the tests listed in [module.yaml](../../docs/modules/contribute-coremind/module.yaml), `npm run test:engineering`, `npm run baseline:check`, `npm run test:stability`, `npm run test:coverage`, `npm run docs:audit`, `npm run acceptance:rc`, and `npm run check:modules`. Keep Windows/Linux coverage floors current and make the generic fallback their per-metric minimum.
9. Engineering CI must not read live-provider credentials. A nightly/manual offline rehearsal is not release qualification. For release-facing changes, require an explicitly selected `strict-provider` run, `Candidate qualified`, both real-pseudoterminal artifacts, commit-bound provider evidence, and the same commit's successful Engineering CI.
