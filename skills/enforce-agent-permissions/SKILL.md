...
8. When testing ask mode, make a deterministic provider repeat the same tool request. Deny the first approval and require one model request, one approval, zero side effects, and a paused outcome; a human denial must never consume later model turns.
...
中文执行原则：先确认业务规则，再按 SOP 实现；失败不得伪装成成功；人工拒绝后当前工具批次必须立即停止并暂停运行，不得继续消耗模型轮次；full 只改变审批强度，不得关闭显式 deny、审计、checkpoint 和恢复。路径感知文件工具与 shell 的平台边界必须分别验证。
