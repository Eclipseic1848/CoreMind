...
`-g` 表示**全局安装**——装一次，之后在任何目录都能用 `coremind` 命令。`0.7.1` 已在 Registry 公开；后续版本仍以 Release 或 Registry 当前列出的版本为准。

### 验证安装成功

```bash
coremind --version        # 0.7.1 显示 coremind v0.7.1
...
```bash
npx -y coremind-cli@0.7.1 doctor
```
...
| 现象 | 原因与解决 |
|---|---|
| `coremind 无法识别` / `command not found` | 没装成功。确认 Registry 已公开目标版本后运行 `npm install -g coremind-cli@0.7.1`；装了还不行 → 重开终端（PATH 刷新）；Windows 上检查 npm 全局目录是否在 PATH |
