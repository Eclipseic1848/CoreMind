...
`test:stability` 连续运行三次全量测试，任何一轮失败立即阻断。`test:coverage` 使用 V8 记录全仓和关键 Runtime 文件的真实覆盖率。Windows 与 Linux 会运行不同的平台安全测试，因此分别锁定全仓不下降基线：Windows 为 lines 72.54%、statements 70.51%、functions 79.82%、branches 62.66%；Linux 为 lines 72.58%、statements 70.48%、functions 80.26%、branches 62.21%。关键 Runtime 文件继续使用跨平台共同底线。当前仍低于全仓 80% 与关键分支 90% 长期目标，因此门禁持续报告差距，不把基线写成目标已达成。
