...
Provider/网络错误只有经统一分类确认为瞬态时才重试。第一次人工审批拒绝会立即终止当前工具批次并暂停运行，不会回灌模型继续请求审批；安全策略拒绝同样暂停。参数错误与确定性业务失败直接失败；中止和超时会传播到 Loop 控制器并留下同名终态。TUI、无头 CLI、TypeScript SDK 和 Python SDK 观察同一状态序列。
