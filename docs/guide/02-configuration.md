...
**内置提供商**：动态继承锁定运行时依赖的全部 Provider。`0.2.0-rc.1` 为 37 个继承入口；`0.3.0-rc.1` 为 39 个继承入口，加上 CoreMind 原生入口共 40 个可配置 Provider。可通过 TypeScript SDK 的 `listInheritedProviders()` 查看当前安装版本的准确清单。继承支持不等于真实认证；没有真实密钥和证据时只能称为可选 Provider。
