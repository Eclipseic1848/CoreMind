...
Results include outcome, evidence references, artifacts, workspace changes, and unresolved risks. Natural-language summaries are not the only result. A successful result is accepted by default after join only when execution is quiescent, ownership is released, and no Effect is started or unknown. A completed committed Effect may be accepted with a successful result, but it does not prove safe redelegation. A failed, cancelled, timed-out, budget-exhausted, or risk-bearing anomalous successful result must first record a Delegation Disposition that accepts the risk, chooses an alternative, safely redelegates, or propagates the terminal outcome.
