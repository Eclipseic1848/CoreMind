...
5. 同一指纹只复用原 ChildRunId；不同指纹返回 conflict。unknown/committed Effect 不得靠新建委派重放。
6. Lease 遗留时按 Workspace Lease 恢复流程显式审计，不删除仍有 Owner 的锁。
7. 运行模块清单中的测试、`npm run check` 与完整仓库门禁。
