...
当前源码修复：Run 结束阶段的 Session 摘要也消耗该 Run 的 turn、token 和 cost 预算，并受剩余 deadline 与取消信号约束。预算不足时跳过摘要、保留历史；请求和 usage 写入事实记录，失败用量未知时不能视作零消耗。验收时对比实际 Provider 请求数与 Run 请求事实数。
