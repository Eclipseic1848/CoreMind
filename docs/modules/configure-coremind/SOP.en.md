...
4. Set iteration, repair, repeated-action, failure, and exhaustion bounds for every Loop. Agent verification requires `passIf`. Published `0.8.0` host verification omits `passIf` and requires [durable host replies](../../../examples/host-verification/README.en.md); unknown is not acceptance.
