...
`0.3.x` 按 A → B → C 三批加固运行时语义，不新增产品行为，不改变 `0.3.0` 的 Config、Protocol、结果语义、权限、副作用、恢复或依赖合同：

- **0.3.x-A：事实、身份与取消收敛**——声明唯一事实与派生投影（Session / Run / Workspace 三个事实域）、类型化身份与关联不变量、取消收敛与输入收据。设计已定稿（术语表、ADR 与规格见 `docs/adr/` 与 `docs/spec/0.3.x-a/`），实现 Issues 已公开（[#35～#42](https://github.com/Eclipseic1848/CoreMind/issues/35)）。
- **0.3.x-B：工具与恢复**——显式工具阶段图与单调安全、持久化故障契约、正交错误结果；Windows 隔离实验仅作为单独授权的 Spike。
- **0.3.x-C：证据系统**——事件回放与真实入口测试、关键模块质量门、Provider 认证矩阵加固、可观测性基线。
...
- **0.4.x**：稳定 Host/Protocol 契约、Session 投影与查询、受控扩展缝隙。
- **0.5.x～0.6.x**：Web 开发环境——运行与控制面先行，再建设在线编辑、测试与评测闭环；始终复用同一 Protocol 与 Runtime。
- **0.7.x**：Goals、Jobs 与子智能体——持久任务、所有权、预算与取消传播。
