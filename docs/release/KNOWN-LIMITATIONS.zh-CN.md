...
> 本文件描述 `0.7.1` 稳定版发布线的能力边界；GitHub Release、npm 与 PyPI 实时页面是安装可用性的唯一依据。

- 当前 40 个 Provider 均可配置，但静态目录不等于真实认证。`0.7.1` 采用[已批准的简化发布](README.zh-CN.md)：双平台离线候选通过，真实 Provider 作业在 npm 安装阶段失败，未执行模型请求，严格认证尚未完成。旧认证证据只供追溯，不代表本版本通过。
- 二期真实外部同题模型评测尚未执行，离线 Coding Eval 不能替代真实模型质量结论。
- `0.7.1` 原样复用 Candidate `33838498153` 的 npm／wheel 和离线证据；清单记录原始构建提交与 `providerCertification: not-run`。任何局部检查通过都不等于公开发布，也不复用 `0.7.0` 的网络例外。
...
- `v0.7.1` Tag、GitHub Release、8 个 npm 包与 PyPI 必须版本一致；包的构建提交与源码 ZIP 的发布提交分别记录在清单中，二者之间不得有产品代码变更。公开渠道是安装可用性的最终依据。
