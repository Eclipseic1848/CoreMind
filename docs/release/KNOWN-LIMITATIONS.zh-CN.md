...
- 当前 40 个 Provider 均可配置，但静态目录不等于真实认证。`0.7.1` 发布只接受与版本、完整提交和 Runtime SHA-256 精确绑定的严格 Provider 资格证据；仓库矩阵中未覆盖当前版本的旧证据只供追溯。
- 二期真实外部同题模型评测尚未执行，离线 Coding Eval 不能替代真实模型质量结论。
- `0.7.1` 的候选包、双平台 TTY、Runtime 摘要、Provider 资格证据与公开发布证据必须精确关联；任何局部检查通过都不等于公开发布。`0.7.0` 的一次性网络例外不能复用于本版本。
...
- `v0.7.1` Tag、GitHub Release、8 个 npm 包与 PyPI 必须指向同一版本与提交；公开渠道是安装可用性的最终依据。
