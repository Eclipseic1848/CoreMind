...
3. `approval-deny`：拒绝第一次写入审批后不再出现同一运行的后续审批，文件没有副作用，终态为 `paused`。
...
认证必须覆盖流式输出、工具调用、结构化结果、多轮会话、中止、错误映射和长上下文七项检查。账号未开通模型服务、权限不足、密钥失效或真实请求失败时停止发布；不得静默换模型、换 Provider 或把历史证据当成本次实时通过。
