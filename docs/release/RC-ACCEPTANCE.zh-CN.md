...
认证必须覆盖流式输出、工具调用、结构化结果、多轮会话、中止、错误映射和长上下文七项检查。账号未开通模型服务、权限不足、密钥失效或真实请求失败时停止发布；不得静默换模型、换 Provider 或把历史证据当成本次实时通过。`0.7.0` 仅接受 Issue #113 已记录、绑定运行 `33582995518` 与固定 Runtime 摘要的一次性网络超时裁决；它不更新认证台账，后续版本恢复严格规则。
