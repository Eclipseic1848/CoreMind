# 0.3.0 Stable Release Known Limitations

> This file records the capability boundary of the published `0.3.0` stable release. Releases and registries remain authoritative for installation and later-version status.

- All 40 provider entries are configurable. `alibaba-model-studio/qwen-plus` completed the seven-check live revalidation on the `0.3.0` release Runtime, while the other 39 remain uncertified. Final status comes from the current-version evidence ledger. Never present configurability as certification or generalize one model's evidence to another model or deployment.
...
- Every publishable tag must bind to the same source that passed P01-P19, Windows/Linux CI, real P20 pseudoterminal acceptance on both platforms, and the live-provider recheck. `v0.3.0` satisfies this constraint; later candidate checks must not be presented as public availability while any release channel lacks the version.
...
- The stable release does not provide a complete Web development environment, hosted API, multi-tenant SaaS, official Docker image, formal macOS support, or extension marketplace.
- GitHub Releases, npm, and PyPI are authoritative for installable availability. `0.3.0` is present in all three channels; later versions must still be checked independently.
