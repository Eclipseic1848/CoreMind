...
Confirmed transient provider and network errors may retry. The first human approval denial terminates the current tool batch and pauses the run without another model request; security denials also pause. Invalid arguments and deterministic business failures fail, while abort or timeout propagates to the Loop controller with the same terminal meaning. TUI, headless CLI, TypeScript SDK, and Python SDK observe the same ordered states.
