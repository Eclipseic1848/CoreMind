...
The stability gate runs the complete suite three consecutive times and stops at the first failure. Vitest multi-project mode does not reliably propagate a root timeout into every independent project, so the long-running Runtime project explicitly uses a 15-second harness limit and CLI subprocess tests use 30 seconds; neither changes product runtime budgets. Windows and Linux execute different platform-security tests, so repository floors are tracked per target: Windows is 72.82% lines, 70.80% statements, 80.32% functions, and 63.30% branches; Linux is 73.26% lines, 71.19% statements, 81.00% functions, and 63.23% branches. Each measurement comes from a candidate run on its target platform. The generic fallback takes the per-metric minimum of the two officially supported platforms and cannot be weaker than either platform floor. Critical Runtime files retain shared cross-platform floors, including 86.23% ToolPolicy branches. Windows shell discovery uses injected deterministic cases, the integration case supplies an explicit minimal environment, property tests use fixed random seeds, and dedicated regressions cover critical permission branches so machine capabilities and random samples cannot drift coverage. The gate rejects every decrease and reports the remaining gaps to the 80% repository and 90% critical-branch long-term targets without claiming they are already met.

The candidate RC check reuses evidence from the three completed Node test runs only when the GitHub run, job, attempt, commit, and platform match. Missing or mismatched evidence fails the check. Standalone `acceptance:rc` still runs the complete Node suite, and coverage remains a separate instrumented run.

The npm gates perform real packing for every public workspace, reject tests, internal plans, run state, checkpoints, temporary files, and credentials, then run publint, type-resolution, and clean-project installation checks. The source ZIP gate snapshots the current candidate through a temporary Git index without changing the real staging area; a cross-platform decoder checks entries and rejects path traversal before clean installation, build, contract checks, and CLI startup. Windows and Linux P20 evidence is recorded separately through a real target-platform pseudoterminal; manual review is required only when automated and visible terminal behavior disagree.
