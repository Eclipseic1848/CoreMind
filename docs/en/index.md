...
The repository's `0.3.2` is currently a draft candidate only. It collects the `0.3.x-B/C` hardening for tools and recovery, Context, Replay, and local observability, plus controlled thinking-disable configuration for Qwen-compatible endpoints. Its current Node 22 both-platform CI, live Provider certification, RC acceptance, and publication remain separate gates.
