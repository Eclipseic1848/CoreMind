...
The `0.3.2` candidate is merged and has completed Node 22 both-platform CI, seven-check live Provider certification, and RC acceptance. No tag, registry artifact, or GitHub Release exists, so `0.3.1` remains the public stable release. Current `main` also contains unreleased `0.4.x` Protocol v2, AgentDriver, and ExecutionEnvironment source.
