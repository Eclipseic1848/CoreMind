...
The current source candidate consistently declares stable `0.7.0`, including Protocol v2, unified security and error contracts, and a Child Run product path across all four formal entries. The merged implementation passes Node 22 engineering CI on both platforms; candidate-package acceptance, live parent-child Provider certification, the tag, registries, and GitHub Release remain incomplete, so `0.3.1` is still the public stable release.
