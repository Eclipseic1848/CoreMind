...
这里不是演示成功文本的静态样例，而是两个可执行的单文件缺陷仓库和两组跨文件 Engineering Kernel 场景。测试会把样例复制到全新的临时 Git 仓库，提交缺陷基线，再制造一份用户未提交草稿，随后运行真实 CoreMind Runtime、工具策略与 schemaVersion 2 grader。
...
跨文件场景分别使用 TypeScript 与 Python，额外验证仓库探测仅提供建议、写前 checkpoint、Diff、Restore、错误命令、审批拒绝和中止。离线门禁合计 6 个 Case。
