...
3. Start the provider：`node ../_shared/mock-provider.mjs contract 8812`。
4. Run in another terminal：`node ../../../packages/coremind-cli/dist/cli.js run coremind.yaml --prompt "服务费用30天支付，未约定责任上限"`。
5. Run evaluation：`node ../../../packages/coremind-cli/dist/cli.js eval coremind.yaml`。

## Expected evidence

The final JSON has riskLevel high and requiresHumanReview true.
