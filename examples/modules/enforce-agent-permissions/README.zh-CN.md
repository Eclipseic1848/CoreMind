...
5. 本配置含 `deny: [bash]`，任何权限模式都必须阻断 `bash`。若单独测试 Windows 宿主 Shell 边界，先在测试专用配置中移除该 deny；ask、assisted、`workspaceOnly: true` 或网络不是 `allow` 的任一情况仍应拒绝，只有 full、关闭工作区限制且允许网络时才执行。Git Bash 或 PowerShell 不是隔离层。
