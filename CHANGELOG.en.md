...
- Added a typed Error Contract registry owned by the Protocol package, covering existing stable CoreMind error codes and their Run-status, cancellation, retry, and human-action classifications. Runtime, Provider, Tool, Child Run, and Evaluation now use that contract. Custom tools explicitly declare expected `tool_execution_failed` results with `ToolExecutionError`; bare Adapter failures and other unknown external codes enter Outcomes and durable Facts only as redacted audit data, while the public code converges to the pausing, non-retryable `unclassified_error`. Protocol, CLI, TUI, and both SDK product entries remain follow-up P0 work.
