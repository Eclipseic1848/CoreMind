...
- Added a typed Error Contract registry owned by the Protocol package, covering existing stable CoreMind error codes and their Run-status, cancellation, retry, and human-action classifications. Unknown external codes may be retained for audit, but the public code converges to the pausing, non-retryable `unclassified_error`. This entry covers only the expand phase; product-entry and historical-Fact migration remain follow-up P0 work.
