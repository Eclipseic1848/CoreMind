...
- Both Ubuntu and Windows candidate matrices passed in strict run `33582995518`. The first `alibaba-model-studio/qwen-plus` request then failed with `provider_transient / Request timed out` before an HTTP response, with no automatic retry and no successful Provider artifact. In Issue #113, the maintainer accepted a `0.7.0`-only network exception bound to the exact run, failed job, and Runtime digest. It is not a live-provider certification and cannot be reused by later releases.
- Added backend-neutral `SecretRef` values and host-injected `SecretResolver`, resolved only at the Provider Adapter boundary. Plaintext credentials fail with `execution_security_violation`; missing environment values, resolvers, or empty results fail with `secret_reference_unresolved` before Provider, Tool, Child Run, or Fact side effects, without fallback or disclosure in errors and persistence.
- Added a single typed Error Contract registry owned by the Protocol package, covering stable CoreMind codes and their Run-status, cancellation, retry, and human-action classifications. Config, Runtime, Provider, Tool, Child Run, Evaluation, Protocol, Worker, CLI, TUI, and both SDKs now use that contract. Protocol v1/v2 schemas, the fingerprint, and public Outcome types derive from the registry, while the Python package ships a generated read-only classification artifact checked entry-for-entry. Custom tools explicitly declare expected `tool_execution_failed` results with `ToolExecutionError`; bare Adapter failures and other unknown external codes enter Outcomes and durable Facts only as redacted audit data and converge publicly to the pausing, non-retryable `unclassified_error`. TypeScript and Python CI gates reject unregistered CoreMind-owned codes.
...
- Config now gives each parent Agent an isolated six-dimensional Delegation Budget pool, with tighten-only defaults of depth 3, four active children, and 32 total descendants. Initialization failures proven to precede the first delegation Fact release reservations; a successful creation or an unknown critical creation-Fact commit retains both identity and budget, with no unused-capacity refund. The same DelegationId and input return the original ChildRunId, while changed input conflicts without a second execution.
...
- The headless CLI now exposes Child Run parent-child identity, target, status, Outcome, and Recovery through human summaries and stable `child_run` JSONL events. Through the bundled Worker, Python observes the same persisted tree in v1 structured results and v2 events/query, without standalone spawn, list, resume, or detach entry points.
- The TUI summary now highlights Child Run count, active descendants, and unhandled risk. Through the read-only `ChatSession.inspectCurrentRunProjection()`, `/children` expands already persisted canonical Facts into the current or terminal tree with identities, targets, budgets, status, Outcome, Recovery, and risk text. `delegate` approval cards prioritize the target, bounded task summary, explicit references, and tightened budget, and state that approval creates only the Child Run rather than pre-approving child Effects; unsupported child-specific disposition controls are not fabricated.
- Every Runtime entry point now shares the delegation approval matrix: `ask` approves every request; `assisted` auto-approves only a Config target explicitly marked `preapproved: true` when every limit passes; and `full` cannot override deny rules, the target allowlist, budgets, non-expanding tools, paths, network, or credentials. CLI/readline and TUI both show the fixed target, bounded task summary, six-dimensional budget, and limits. Approval Facts carry both the argument fingerprint and the exact Child Run input fingerprint used by the creation Fact, while the child retains independent ToolPolicy and Effect approval Facts.
- Cross-process acceptance covers bounded cancellation and eventual join by the formal Delegation Tool using the Child wall-time limit; Host-crash orphan audit followed by Protocol v2 tree recovery from the same Fact prefix; Child Worker crash; no surviving child process after parent cancellation; no Provider or Effect replay; and a single Writer under parent-child workspace contention. The merged P0 implementation and the strict candidate's Node 22 engineering, package, and real-TTY gates pass on both platforms. Durable detach remains unsupported.
- The root version, eight npm packages, exact internal dependencies, Python SDK, bundled Worker, module contracts, documentation, and Release Please metadata agree on stable `0.7.0`. Source development and CI pin npm `11.5.1` so a retired legacy audit endpoint cannot reject a valid workspace tree. The `v0.7.0` tag, [GitHub Release](https://github.com/Eclipseic1848/CoreMind/releases/tag/v0.7.0), eight npm packages, and [PyPI package](https://pypi.org/project/coremind-ai/0.7.0/) are published at the same version.
- The phase gate updates the bilingual README and security policy. The Code of Conduct, contributing guide, and MIT License have no changed facts, process, or contacts and remain unchanged.
